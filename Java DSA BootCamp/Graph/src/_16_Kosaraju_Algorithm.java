...
public class _16_Kosaraju_Algorithm {
    public static class Edge11 {
        ...
        public Edge11(int s, int d) {
            this.src = s;
            this.dest = d;
        }
    }

    public static void CreateGraph(ArrayList<Edge11>[] graph) {
        for (int i = 0; i < graph.length; i++) {
            graph[i] = new ArrayList<Edge11>();
        }
        graph[0].add(new Edge11(0, 2));
        graph[0].add(new Edge11(0, 3));
        graph[1].add(new Edge11(1, 0));
        graph[2].add(new Edge11(2, 1));
        graph[3].add(new Edge11(3, 4));
    }

    public static void DFS(ArrayList<Edge11> graph[], int curr, boolean vis[]) {
        vis[curr] = true;
        System.out.print(curr + " ");
        for (int i = 0; i < graph[curr].size(); i++) {
            Edge11 e = graph[curr].get(i);
            if (!vis[e.dest]) {
    ...
    public static void toposort(ArrayList<Edge11> graph[], int curr, boolean vis[], Stack<Integer> s) {
        vis[curr] = true;
        for (int i = 0; i < graph[curr].size(); i++) {
            Edge11 e = graph[curr].get(i);
            if (!vis[e.dest]) {
    ...
    public static void kosaraju(ArrayList<Edge11> graph[], int v, boolean vis[]) {
        // Sorting the Nodes
        ...
        ArrayList<Edge11> transpose[] = new ArrayList[v];
        for (int i = 0; i < transpose.length; i++) { // This loop makes the Array List null and Visited array also initialize with false to us to reuse in DFS on Transpose Graph
            vis[i] = false;
            transpose[i] = new ArrayList<Edge11>();
        }
        for (int i = 0; i < v; i++) {
            for (int j = 0; j < graph[i].size(); j++) {
                Edge11 e = graph[i].get(j);
                transpose[e.dest].add(new Edge11(e.dest, e.src)); // This Line creates and Transpose graph
            }
        }
    ...
    public static void main(String[] args) {
        int v = 5;
        ArrayList<Edge11>[] graph = new ArrayList[v];
        CreateGraph(graph);
