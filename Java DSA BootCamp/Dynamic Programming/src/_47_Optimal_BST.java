// https://youtu.be/wAy6nDMPYAE?si=OkojelskqNkvSfaY
public class _47_Optimal_BST {
        ...
        for (int lengthOfBlock = 2; lengthOfBlock <= n; lengthOfBlock++) {
            for (int start = 0; start <= n - lengthOfBlock + 1; start++) {
                ...
                for (int root = start; root <= end; root++) {
                    ...
                    if (currentCost < cost[start][end]) cost[start][end] = currentCost;
                }
            }
        }
        return cost[0][n - 1];
    ...
    public static void main(String[] args) {
        int[] freq = {34, 8, 50};
        int[] keys = {10, 12, 20};
        System.out.println("Optimal BST cost " + optimalSearchTree(freq, keys.length));
    }
}
