...
public class LFU_Cache { // Least Frequently Used Cache
    final int capacity;
    int curSize;
    int minFrequency;
    ...
    static class DoubleLinkedList {
        ...
        public void addToHead(DLLNode node) { // Add a node to the head of the linked list
            node.next = head.next;
            node.prev = head;
            head.next.prev = node;
            head.next = node;
    ...
    public void put(int key, int value) {
        ...
        if (cache.containsKey(key)) { // If key exists, update the value and increase the frequency
            DLLNode curNode = cache.get(key);
            curNode.value = value;
            updateNode(curNode);
        } else {
            if (curSize == capacity) { // If the cache is full, remove the least frequently used element
                evictNode();
            }
    ...
    private void updateNode(DLLNode node) {
        DoubleLinkedList curList = frequencyMap.get(node.frequency); // Remove the node from its current frequency list
        curList.removeNode(node);
        if (curList.isEmpty() && node.frequency == minFrequency) { // If the current list is empty and its frequency is the minFrequency, update minFrequency
            minFrequency++;
        }
    ...
    private void addToFrequencyList(DLLNode node) {
        DoubleLinkedList curList = frequencyMap.getOrDefault(node.frequency, new DoubleLinkedList()); // Get the list for the current frequency, or create a new list if it doesn't exist
        curList.addToHead(node);
        frequencyMap.put(node.frequency, curList);
    }

    private void evictNode() {
        DoubleLinkedList minFrequencyList = frequencyMap.get(minFrequency); // Get the list with the minimum frequency
        DLLNode evictedNode = minFrequencyList.tail.prev; // Get the least frequently used node from the list
