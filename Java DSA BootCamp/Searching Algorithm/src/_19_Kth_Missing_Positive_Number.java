public class _19_Kth_Missing_Positive_Number {
    ...
    public static int findRecursive(int[] arr,int k,int l,int h){
        if(l<=h){
            int mid=l+(h-l)/2;
            if(arr[mid]-(mid+1)>k) return findRecursive(arr,k,l,mid-1);
            else return findRecursive(arr,k,mid+1,h);
        }
        return l+k;
    }
    public static void main(String[] args) {
        ...
        System.out.println(findRecursive(arr,k,1,n-2));
    }
}
